...
if [[ ${COMP_WORDS[1]} = "wait" ]] && [[ ${#COMP_WORDS[@]} -eq 3 ]]; then
	...
	COMPREPLY=($(compgen -W "$(export TODOTXT_PLAIN=1; eval $our_cmd ls \
		| awk '/^[0-9]/{print $1}' | tr -d '\r' | sort -n)" -- "${word}"));
fi
