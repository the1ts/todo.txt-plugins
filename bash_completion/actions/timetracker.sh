...
if [[ ${COMP_WORDS[1]} =~ ^(timetracker|tt)$ ]]; then
	...
	if ((cmd == 2)); then
		...
		off)
			# shellcheck disable=SC2207,SC2086
			mapfile -t COMPREPLY < <(compgen -W "$(eval ${our_cmd} timetracker simplelistallstarted)" -- "${word}")
			;;
		archive)
			# shellcheck disable=SC2207,SC2086
			mapfile -t COMPREPLY < <(compgen -W "$(eval ${our_cmd} timetracker simplelistallstarted)" -- "${word}")
		...
		stats)
			...
			mapfile -t COMPREPLY < <(compgen -W "$({
				eval ${our_cmd} timetracker simplelistcompleted
				eval ${our_cmd} timetracker simpleliststarted
			} | sort -u)" -- "${word}")
			;;
		archivedstats)
			# shellcheck disable=SC2207,SC2086
			mapfile -t COMPREPLY < <(compgen -W "$(eval ${our_cmd} timetracker simplelistarchived | sed 's/^/+/g' | sort -u)" -- "${word}")
			;;
		esac
	fi
fi
